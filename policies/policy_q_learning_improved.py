...
class QLearningAgent(bp.Policy):
    ...
    def act(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        if self.mode == 'test':
            ...
            new_state, prev_action, prev_state, reward = self.handle_and_store_input_ignore_none(new_state, prev_action, prev_state,
                                                                                                 reward)
    ...
    def play(self, new_state):
        return self.session.run(self.output_argmax,
                                feed_dict={self.input: reshape_double_board(new_state)})[0]

    def handle_and_store_input_ignore_none(self, new_state, action, prev_state, reward):
        new_state = reshape_double_board(new_state)
        if prev_state is None:
            return reshape_double_board(new_state), action, prev_state, reward
